...
RESOLVE_MODULES_PATH = os.environ.get("PYTHONPATH") or paths["modules_path"]
...
# 必须把该目录加到 PATH 并切换工作目录，否则初始化会失败
if RESOLVE_LIB_PATH:
...
def _load_resolve_script():
    ...
    if dvr_script is not None:
        return True
    try:
        import DaVinciResolveScript as dvr_script_module
...
def _launch_resolve():
    ...
    if sys_name == "darwin":
        ...
        # Use RESOLVE_LIB_PATH to derive Resolve.exe location (supports non-C: installs)
        if RESOLVE_LIB_PATH:
