...
# fusionscript.dll 需要 PYTHONHOME 来定位 Python 标准库
if not os.environ.get("PYTHONHOME"):
    _base = sys.base_prefix or sys.prefix
    if _base:
        os.environ["PYTHONHOME"] = _base

# fusionscript.dll 依赖 Resolve 安装目录中的其他 DLL
if RESOLVE_LIB_PATH:
    _resolve_bin_dir = os.path.dirname(RESOLVE_LIB_PATH)
    if os.path.isdir(_resolve_bin_dir):
        os.environ["PATH"] = _resolve_bin_dir + os.pathsep + os.environ.get("PATH", "")
        os.chdir(_resolve_bin_dir)
...
def _load_resolve_script():
    """Lazy-load DaVinciResolveScript module (only when first needed)."""
    global dvr_script
    if dvr_script is not None:
        return True
    try:
        import DaVinciResolveScript as dvr_script_module  # type: ignore
        dvr_script = dvr_script_module
...
def _connect_resolve():
    """Lazy connect to Resolve. Returns resolve object or None."""
    global resolve, dvr_script
    if resolve is not None:
        return resolve
    if not _load_resolve_script():
        return None
    try:
        candidate = dvr_script.scriptapp("Resolve")
        if candidate:
            logger.info(
                f"Connected to DaVinci Resolve: {candidate.GetProductName()} {candidate.GetVersionString()}"
            )
            resolve = candidate
        else:
...
def _launch_resolve():
    ...
    if sys_name == "darwin":
        ...
        if RESOLVE_LIB_PATH:
            _resolve_exe = os.path.join(os.path.dirname(RESOLVE_LIB_PATH), "Resolve.exe")
        else:
            _resolve_exe = r"C:\Program Files\Blackmagic Design\DaVinci Resolve\Resolve.exe"
        app_path = _resolve_exe
        if not os.path.exists(app_path):
            return False
