...
def find_resolve_paths():
    ...
    Priority: environment variables > predefined paths > drive scan (Windows).
    """
    ...
    # Priority 2: Hardcoded paths / drive scan
...
def build_server_env(python_path, api_path, lib_path, system=SYSTEM, python_home=None):
    ...
    # fusionscript.dll 需要 PYTHONHOME 来定位 Python 标准库（Windows）
    if system == "Windows":
        env["PYTHONHOME"] = str(python_home or get_python_base_install(python_path))

    # fusionscript.dll 需要 Resolve 安装目录中的其他 DLL 才能初始化
    if lib_value:
        _resolve_bin_dir = str(Path(lib_value).parent)
        if os.path.isdir(_resolve_bin_dir):
            env["PATH"] = _resolve_bin_dir + os.pathsep + "${PATH}"
...
def verify_resolve_connection(python_path, api_path, lib_path):
    ...
    lib_dir = str(Path(lib_path).parent) if lib_path else ""
    test_script = textwrap.dedent(f"""\
        import os, sys
        sys.path.insert(0, {modules_path!r})
        # fusionscript.dll needs its sibling DLLs and PYTHONHOME
        lib_dir = {lib_dir!r}
...
def print_banner():
    ...
    subtitle = "27 compound · 354 full · 3 platforms"
    print()
    print(bold("  ╔══════════════════════════════════════════════════════╗"))
...
def main():
    ...
    if api_path:
        ...
        if is_linux():
            ...
            print(f"  {dim('Tip: Set RESOLVE_SCRIPT_API env var pointing to your Scripting folder,')}")
            print(f"  {dim('     e.g. D:\\ProgramData\\Blackmagic Design\\DaVinci Resolve\\Support\\Developer\\Scripting')}")
    ...
    if lib_path:
        ...
        if is_windows():
            print(f"  {dim('     Set RESOLVE_SCRIPT_LIB to the fusionscript.dll path (e.g.')}")
            print(f"  {dim('     D:\\Program Files\\Blackmagic Design\\DaVinci Resolve\\fusionscript.dll)')}")
